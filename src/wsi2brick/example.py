...
from wsi2brick import convert_tile
...
def init_argparse() -> ArgumentParser:
    ...
    parser.add_argument(
        'filename', help='The whole slide image to convert', type=str)
    parser.add_argument('output', help='The output directory', type=str)
    parser.add_argument('-s', '--tile-size',
                        help='The size of the tiles', default=32, type=int)
    parser.add_argument(
        '-m', '--mode',
...
if __name__ == '__main__':
    ...
    if args.mode == 'wsi':
        ...
        ids, brick = convert_tile(
            OpenSlide(args.filename), tuple(args.region), args.tile_size)
        print(ids)
